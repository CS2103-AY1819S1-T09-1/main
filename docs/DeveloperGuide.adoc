...
** Cons: Poorer performance, although this is insignificant since number of elements to be created/deleted is small and fixed.
...
** Cons: There were some difficulties around creating a separate `UiPart` component that could be added as a child of a `VBox` element.
// end::calendarshow[]
