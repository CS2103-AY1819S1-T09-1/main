...
==== Future Enhancements
Currently, the method `void updateTask(Task target, Task editedTask)` and `void updatePerson(Person target, Person editedPerson)` in the `Model` interface are very similar. Perhaps in the future `Model` can be refactored as a generic so that there is minimum code duplication.
