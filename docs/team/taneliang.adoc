...
Cow is a desktop group project task manager aimed at students. Some features include individual task management, and project groupmate management and task assignment. The user interacts with it using a CLI, and it has a GUI created with JavaFX. It is written in Java, and has about 15 kLoC.
...
* *Minor enhancement*: added a history command that allows the user to navigate to previous commands using up/down keys. **TO CHANGE**
